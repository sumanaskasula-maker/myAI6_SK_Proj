...
# 390 Adventure X — Quantitative Specifications (KTM)

This document is the structured, numeric-spec companion to the qualitative narrative document for the 390 Adventure X. Every field below is drawn directly from the KTM master dataset (all 1 trim(s)/variant(s) on record for this model). No field has been omitted; specs identical across all trims are stated once below, and specs that differ by trim are listed in the per-trim table.
...
- **Segment:** 390cc adventure-touring (road-biased, alloy-wheel trim)
...
- **Single trim (as listed):** power-to-weight ratio ≈ 186.9 W/kg (0.254 PS/kg).
